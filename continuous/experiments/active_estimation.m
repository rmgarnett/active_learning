...
[purely_random_estimated_proportion purely_random_proportion_variance ...
 purely_random_chosen purely_random_responses] = ...
    purely_random_surveying_continuous(sampling_function, ...
...
gp_response_function = @(x) 2 * response_function(x) - 1;

proportion_estimation_function = @(data, responses) ...
    gp_estimate_proportion_continuous(data, responses, choose_samples(), ...
        inference_method, mean_function, covariance_function, ...
        likelihood, hypersamples, num_f_samples);

utility_function = @(data, responses, test) random_utility(data, ...
        responses, test);

[random_estimated_proportions random_proportion_variances random_chosen ...
 random_responses] = iterative_surveying_continuous(data, responses, ...
        gp_response_function, optimization_function, utility_function, ...
...
probability_function = @(data, responses, test) ...
    gp_probability_continuous(data, responses, test, inference_method, ...
                              mean_function, covariance_function, ...
                              likelihood, hypersamples);

utility_function = @(data, responses, test) ...
    uncertainty_utility_continuous(data, responses, test, probability_function);

[uncertainty_estimated_proportions uncertainty_proportion_variances ...
 uncertainty_chosen] = iterative_surveying_continuous(data, responses, ...
        gp_response_function, optimization_function, utility_function, ...
        proportion_estimation_function, num_evaluations);
